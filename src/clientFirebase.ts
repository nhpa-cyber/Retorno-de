...
export function getActiveFirebaseConfig(): any {
  if (typeof window !== "undefined") {
    try {
      ...
      localStorage.removeItem("db_schedule_auto_enabled");
      localStorage.removeItem("db_custom_schedule_rules");
    } catch (e) {}
  }
  return FIREBASE_PRESETS[0].config;
}
 ...
 */
export function subscribeToFirestore(onUpdate: (db: any, changedCollection?: string) => void): () => void {
  const db = getClientFirestore();
  ...
  const notifyCollectionUpdate = (targetCol: string, data: any) => {
    if (debounceTimers[targetCol]) clearTimeout(debounceTimers[targetCol]);
    debounceTimers[targetCol] = setTimeout(() => {
      onUpdate({ [targetCol]: data, ...(targetCol === 'auditLogs' ? { audit_logs: data } : {}) }, targetCol);
      delete debounceTimers[targetCol];
    }, 25);
  };
    ...
    try {
      if (colName === "customManual") {
        ...
        const unsub = onSnapshot(docRef, (docSnap) => {
          ...
          const manualContent = docSnap.exists() ? (docSnap.data().html || docSnap.data().content || "") : "";
          combinedDb.customManual = manualContent;
          notifyCollectionUpdate("customManual", manualContent);
        ...
        const unsub = onSnapshot(collRef, (snapshot) => {
          ...
          if (snapshot.empty) {
            ...
            } else if (colName === "operationalActions" && DEFAULT_OPERATIONAL_ACTIONS.length > 0) {
              saveDocsToFirestore("operationalActions", DEFAULT_OPERATIONAL_ACTIONS);
            }
          }
          ...
          notifyCollectionUpdate(targetCol, items);
        }, (error) => handleSubscriptionError(error));
        unsubscribes.push(unsub);
      }
    } catch (err) {
...
export function subscribeToCollection(colName: string, onUpdate: (items: any[]) => void): () => void {
  const db = getClientFirestore();
  if (!db || hasClientPermissionError) return () => {};
