...
import { DEFAULT_PRODUCTS, DEFAULT_USERS, DEFAULT_ACTIVE_ASSETS, DEFAULT_DRIVERS, DEFAULT_VEHICLES, DEFAULT_VALES } from './data';
...
export default function App() {
  ...
  const [vales, setVales] = useState<Vale[]>(() => DEFAULT_VALES);
  ...
  // Pre-indexed map for O(1) product description repairs
  const defaultProductsMap = React.useMemo(() => {
    const map = new Map<string, string>();
    DEFAULT_PRODUCTS.forEach(dp => {
      if (dp.code && dp.description) {
        map.set(dp.code, dp.description);
      }
    });
    return map;
  }, []);
  ...
  const repairProductsList = (list: Product[]) => {
    ...
    return list.map(p => {
      if (p.description === p.code || !p.description || p.description.trim() === '') {
        const originalDesc = defaultProductsMap.get(p.code);
        if (originalDesc) {
          return { ...p, description: originalDesc };
        }
      }
      return p;
    });
  ...
  // Helper to ensure active assets have latest baseline costs from factory table
  const defaultAssetsMap = React.useMemo(() => {
    const map = new Map<string, ActiveAsset>();
    DEFAULT_ACTIVE_ASSETS.forEach(da => {
      map.set(da.id, da);
      if (da.name) map.set(da.name.toUpperCase(), da);
    });
    return map;
  }, []);

  const repairActiveAssetsList = (list: ActiveAsset[]): ActiveAsset[] => {
    if (!list || list.length === 0) return DEFAULT_ACTIVE_ASSETS;
    // Merge existing assets with latest baseline costs
    const existingIds = new Set(list.map(a => a.id));
    const merged = list.map(a => {
      const def = defaultAssetsMap.get(a.id) || defaultAssetsMap.get(a.name?.toUpperCase());
      if (def && (a.cost === undefined || a.cost === null || a.cost === 31.16 || a.cost === 35.0 || a.cost === 25.0 || a.cost === 2.5 || a.cost === 2.2 || a.cost === 1.5 || a.cost === 120.0 || a.cost === 18.0)) {
        return { ...a, cost: def.cost, category: def.category || a.category };
      }
      return a;
    });

    // Add any missing default assets
    DEFAULT_ACTIVE_ASSETS.forEach(da => {
  ...
  const repairDriversList = (list: Driver[]): Driver[] => {
    if (!list || list.length === 0) return DEFAULT_DRIVERS;
    const validDrivers = list.filter(d => {
    ...
    const existingIds = new Set(validDrivers.map(d => d.id.toUpperCase()));
    const merged = [...validDrivers];
    DEFAULT_DRIVERS.forEach(dd => {
      if (!existingIds.has(dd.id.toUpperCase())) {
        merged.push(dd);
      }
    });
    return merged;
  ...
  const repairVehiclesList = (list: Vehicle[]): Vehicle[] => {
    if (!list || list.length === 0) return DEFAULT_VEHICLES;
    // Garante estritamente apenas os 17 veículos oficiais da frota
  ...
  const cleanVales = (list: Vale[]): Vale[] => {
    const baseList = (list && list.length > 0) ? list : DEFAULT_VALES;
    const map = new Map<string, Vale>();
    DEFAULT_VALES.forEach(v => map.set(v.id, v));
    baseList.forEach(v => {
  ...
  const getRouteStatusRank = (status?: string): number => {
    switch (status) {
      case 'fechado': return 6;
      case 'em_analise': return 5;
      case 'reconferir': return 4;
      case 'conferindo': return 3;
      case 'descarregado': return 2;
      case 'descarregando': return 2;
      case 'pendente':
      default: return 1;
    }
  };
  ...
  const applyDirectDb = (db: any, changedCollection?: string) => {
    if (!db) return;
    ...
    const shouldProcess = (colKey: string) => !changedCollection || changedCollection === colKey || (changedCollection === 'auditLogs' && colKey === 'audit_logs');

    if (shouldProcess('users') && db.users !== undefined && Array.isArray(db.users)) {
      const activeUsers = db.users.length > 0 ? db.users : DEFAULT_USERS;
      setUsers(prev => (prev.length === activeUsers.length ? prev : activeUsers));
      const savedUserId = localStorage.getItem('logiroute_authenticated_user_id');
      ...
      if (matchedUser) {
        setCurrentUser(prev => (prev?.id === matchedUser!.id && prev?.role === matchedUser!.role && prev?.name === matchedUser!.name ? prev : matchedUser));
        localStorage.setItem('logiroute_authenticated_user_id', matchedUser.id);
      }
    }

    if (shouldProcess('drivers') && db.drivers !== undefined && Array.isArray(db.drivers)) {
      const repaired = repairDriversList(db.drivers);
      setDrivers(prev => (prev.length === repaired.length ? prev : repaired));
    }

    if (shouldProcess('vehicles') && db.vehicles !== undefined && Array.isArray(db.vehicles)) {
      const repaired = repairVehiclesList(db.vehicles);
      setVehicles(prev => (prev.length === repaired.length ? prev : repaired));
    }

    if (shouldProcess('products') && db.products !== undefined && Array.isArray(db.products)) {
      const repaired = repairProductsList(db.products);
      setProducts(prev => (prev.length === repaired.length ? prev : repaired));
    }

    if (shouldProcess('activeAssets') && db.activeAssets !== undefined && Array.isArray(db.activeAssets)) {
      const repairedAssets = repairActiveAssetsList(db.activeAssets);
      setActiveAssets(prev => (prev.length === repairedAssets.length ? prev : repairedAssets));
    }

    if (shouldProcess('audits') && db.audits !== undefined && Array.isArray(db.audits)) {
      const cleaned = cleanAudits(db.audits);
      setAudits(cleaned);
    }

    if (shouldProcess('vales') && db.vales !== undefined && Array.isArray(db.vales)) {
      const cleaned = cleanVales(db.vales);
      setVales(cleaned);
    }

    if (shouldProcess('returnForecasts') && db.returnForecasts !== undefined && Array.isArray(db.returnForecasts)) {
      const cleaned = cleanReturnForecasts(db.returnForecasts);
      setReturnForecasts(cleaned);
    }

    if (shouldProcess('fiscalAlerts') && db.fiscalAlerts !== undefined && Array.isArray(db.fiscalAlerts)) {
      setFiscalAlerts(db.fiscalAlerts);
    }

    // Smart Merge Imported Routes
    if (shouldProcess('importedRoutes') && db.importedRoutes !== undefined) {
      const remoteCleaned = cleanImportedRoutes(db.importedRoutes, db.audits);
      setImportedRoutes(remoteCleaned);
    }

    if ((shouldProcess('auditLogs') || shouldProcess('audit_logs')) && (db.audit_logs || db.auditLogs)) {
      const logs = db.audit_logs || db.auditLogs;
      setAuditLogs(prev => (prev.length === logs.length ? prev : logs));
    }

    if (shouldProcess('customManual') && db.customManual !== undefined) {
      const manualContent = typeof db.customManual === 'string' ? db.customManual : db.customManual?.html || '';
      setCustomManualHTML(prev => (prev === manualContent ? prev : manualContent));
    }
  };
  ...
  // Broadcast partial database updates instantly across tabs of the SAME browser
  const broadcastToOtherTabs = (payload: {
    audits?: AuditSession[];
    importedRoutes?: ImportedRoute[];
    returnForecasts?: ReturnForecast[];
    fiscalAlerts?: FiscalAlert[];
    ...
    if (typeof window !== 'undefined' && 'BroadcastChannel' in window) {
      try {
        const channel = new BroadcastChannel('logiroute_realtime_sync');
        channel.postMessage({
          type: 'DB_PARTIAL_SYNC',
          payload
        });
        channel.close();
      } catch (e) {}
    ...
    if (channel) {
      channel.onmessage = (event) => {
        if (event.data && event.data.type === 'RESET_PLATFORM') {
          reloadServerState();
        } else if (event.data && event.data.type === 'DB_PARTIAL_SYNC' && event.data.payload) {
          const pld = event.data.payload;
          if (pld.importedRoutes) {
            setImportedRoutes(cleanImportedRoutes(pld.importedRoutes));
          }
          if (pld.audits) {
            setAudits(cleanAudits(pld.audits));
          }
          if (pld.returnForecasts) {
            setReturnForecasts(cleanReturnForecasts(pld.returnForecasts));
          }
          if (pld.fiscalAlerts) {
            setFiscalAlerts(pld.fiscalAlerts);
          }
          if (pld.vales) {
            setVales(cleanVales(pld.vales));
          }
          if (pld.users) {
            setUsers(pld.users);
          }
          if (pld.drivers) {
            setDrivers(pld.drivers);
          }
          if (pld.vehicles) {
            setVehicles(pld.vehicles);
          }
          if (pld.products) {
            setProducts(pld.products);
          }
          if (pld.activeAssets) {
            setActiveAssets(pld.activeAssets);
          }
        }
      };
    }
  ...
  */
  ...
  const handleSaveAudits = useCallback((newAudits: AuditSession[]) => {
    ...
    const oldAuditsMap = new Map<string, AuditSession>();
    for (let i = 0; i < audits.length; i++) {
      oldAuditsMap.set(audits[i].id, audits[i]);
    }

    const updatedAudits = newAudits.map(newAudit => {
      const oldAudit = oldAuditsMap.get(newAudit.id);
      if (!oldAudit) {
        return {
          ...newAudit,
          updatedAt: timestamp,
      ...
      const isChanged = oldAudit.status !== newAudit.status ||
        oldAudit.routeMap !== newAudit.routeMap ||
        oldAudit.driverId !== newAudit.driverId ||
        oldAudit.plate !== newAudit.plate ||
        oldAudit.surplusFlowStatus !== newAudit.surplusFlowStatus ||
        oldAudit.reopeningRequested !== newAudit.reopeningRequested ||
        oldAudit.pdfDownloaded !== (newAudit as any).pdfDownloaded ||
        oldAudit.financeiroCiente !== (newAudit as any).financeiroCiente ||
        oldAudit.gestorAcknowledgedSurplus !== (newAudit as any).gestorAcknowledgedSurplus ||
        oldAudit.items.length !== newAudit.items.length ||
        oldAudit.assets.length !== newAudit.assets.length ||
        (oldAudit.history?.length || 0) !== (newAudit.history?.length || 0);
    ...
    setAudits(cleaned);
    broadcastToOtherTabs({ audits: cleaned });
    pushDatabaseToServer({ audits: cleaned });
  }, [audits, currentUser]);

  const handleSaveForecasts = useCallback((newForecasts: ReturnForecast[]) => {
    const cleaned = cleanReturnForecasts(newForecasts);
    setReturnForecasts(cleaned);
    broadcastToOtherTabs({ returnForecasts: cleaned });
    pushDatabaseToServer({ returnForecasts: cleaned });
  }, []);

  const handleSaveAlerts = useCallback((newAlerts: FiscalAlert[]) => {
    setFiscalAlerts(newAlerts);
    broadcastToOtherTabs({ fiscalAlerts: newAlerts });
    pushDatabaseToServer({ fiscalAlerts: newAlerts });
  }, []);

  const handleSaveImportedRoutes = useCallback((newRoutes: ImportedRoute[]) => {
    const timestamp = new Date().toISOString();
    const oldRoutesMap = new Map<string, ImportedRoute>();
    for (let i = 0; i < importedRoutes.length; i++) {
      const r = importedRoutes[i];
      const key = `${normalizeMapCode(r.routeMap).toUpperCase()}_${r.routeDate || ''}`;
      oldRoutesMap.set(key, r);
    }

    const updatedRoutes = newRoutes.map(newRoute => {
      const newMapKey = `${normalizeMapCode(newRoute.routeMap).toUpperCase()}_${newRoute.routeDate || ''}`;
      const oldRoute = oldRoutesMap.get(newMapKey);
      if (!oldRoute) {
        return {
          ...newRoute,
          updatedAt: timestamp
      ...
      const isChanged = oldRoute.status !== newRoute.status ||
        oldRoute.driverId !== newRoute.driverId ||
        oldRoute.plate !== newRoute.plate ||
        oldRoute.pallets !== newRoute.pallets ||
        oldRoute.itemsCount !== newRoute.itemsCount ||
        oldRoute.isPernoite !== newRoute.isPernoite ||
        oldRoute.loadingStatus !== newRoute.loadingStatus ||
        oldRoute.isBlitz !== newRoute.isBlitz ||
        oldRoute.discrepancyObservation !== newRoute.discrepancyObservation;
    ...
    broadcastToOtherTabs({ importedRoutes: cleaned });
    pushDatabaseToServer({ importedRoutes: cleaned });
  }, [importedRoutes, audits]);

  const handleSaveVales = useCallback((newVales: Vale[]) => {
    const cleaned = cleanVales(newVales);
    setVales(cleaned);
    broadcastToOtherTabs({ vales: cleaned });
  ...
  const handleUserChange = (user: User) => {
    ...
    } else if (user.role === 'empilhador') {
      setActiveTab('empilhador_view');
