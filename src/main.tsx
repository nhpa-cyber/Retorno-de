...
function RootApp() {
  ...
  useEffect(() => {
    const errorHandler = (event: ErrorEvent) => {
      // Ignore non-fatal cross-origin "Script error." thrown by browser when cross-domain/iframe scripts fail
      if (event.message === "Script error." || (event.error && String(event.error).includes("Script error."))) {
        console.warn("Ignored non-fatal cross-origin Script error:", event);
        return;
      }
