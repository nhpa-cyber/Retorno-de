...
export function processRefugoImportData(
  ...
  for (let rowIdx = 0; rowIdx < rows.length; rowIdx++) {
    ...
    for (const key of rowKeys) {
      ...
      } else if (keyUpper.includes('AFERIDO') || keyUpper.includes('TOTAL_AFERIDO') || keyUpper.includes('VOLUME') || keyUpper.includes('CAIXAS')) {
        let vNum = 0;
        if (typeof val === 'number') vNum = val;
        else if (typeof val === 'string') {
          const parsed = parseFloat(val.replace(',', '.'));
          if (!isNaN(parsed)) vNum = parsed;
        }
        if (vNum > 0) rawVolume += Math.round(vNum);
      }
    ...
    if (rawVolume > 0) {
      mapGroups[mapClean].volumeAferido += rawVolume;
    }
  ...
  for (const [mapCode, group] of Object.entries(mapGroups)) {
    ...
    const auditDoc: AuditSession = {
      ...
      isEstimated: true,
      estimatedVolumeHandled: group.volumeAferido > 0 ? group.volumeAferido : undefined,
