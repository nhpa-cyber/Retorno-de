...
import { isClientFirebaseActive, getActiveFirebaseConfig } from '../clientFirebase';
...
export default function LoginView({ users, onLoginSuccess }: LoginViewProps) {
  ...
  useEffect(() => {
    ...
    const loadFirebaseConfig = async () => {
      ...
      // Fallback to active preset or default config
      if (!cfg || !cfg.apiKey) {
        cfg = getActiveFirebaseConfig() || FIREBASE_PRESETS[0]?.config;
      }
